...
export default function OutlinesPage() {
  ...
  // Store usage
  const { syllabi, loading, error, fetchSyllabi, deleteOutline } = useOutlineStore();
  ...
  const confirmDelete = async () => {
    ...
    const success = await deleteOutline(deletingId);
    if (success) {
      toast({ title: 'Success', description: t('messages.deleteSuccess') });
    } else {
      toast({ title: 'Error', description: t('messages.deleteError'), variant: 'destructive' });
    }
    setDeletingId(null);
  };
        ...
        <div className="max-w-7xl mx-auto flex flex-col gap-12">
          ...
          <div className="flex justify-between items-end">
             ...
             <div className="flex gap-4">
               <Button onClick={fetchSyllabi} variant="outline" className="rounded-2xl h-14 w-14 p-0 border-[#EDEEEF] bg-white shadow-xl shadow-black/5">
                 <RefreshCcw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
               </Button>
               <Button onClick={() => setIsCreating(true)} className="bg-[#000666] hover:bg-[#1A237E] text-white rounded-2xl h-14 px-8 font-black uppercase tracking-[0.2em] text-[10px] shadow-2xl shadow-indigo-900/20 flex items-center gap-3">
          ...
          <div className="grid grid-cols-3 gap-8">
             {stats.map((stat, i) => (
               <div key={i} className="bg-white rounded-3xl p-8 shadow-xl shadow-black/5 flex items-center gap-6 group hover:-translate-y-1 transition-all border-none">
          ...
          {error && (
             <div className="bg-rose-50 border border-rose-100 rounded-[32px] p-10 flex flex-col items-center gap-4">
                ...
                <Button onClick={fetchSyllabi} className="mt-4 bg-rose-600 hover:bg-rose-700 text-white rounded-xl font-black uppercase tracking-widest text-[10px] px-8 h-12">
                   Retry Connection
                </Button>
             </div>
          )}
              ...
              {syllabi.map((item) => {
                ...
                return (
                  <div key={item.id} className="bg-white rounded-[40px] p-10 shadow-2xl shadow-black/[0.03] flex flex-col gap-8 group hover:shadow-indigo-900/5 hover:-translate-y-1 transition-all border border-transparent hover:border-[#1A237E]/10 relative overflow-hidden">
                    <div className={`absolute top-0 right-10 h-1.5 w-20 rounded-b-full ${status === 'Active' ? 'bg-emerald-500' : 'bg-amber-400'}`} />
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-2">
                         <button onClick={() => setDeletingId(item.id)} className="p-2 rounded-full hover:bg-rose-50 text-[#767683] hover:text-rose-500 transition-colors">
                            <Trash2 className="w-5 h-5" />
                         </button>
                      </div>
                    </div>

                    <p className="text-sm font-medium text-[#767683] leading-relaxed line-clamp-2 h-10 pr-12">
                    ...
                    <div className="flex items-center gap-8 py-2 border-y border-[#EDEEEF]/50">
                      ...
                      <div className="flex flex-col gap-1 border-l border-[#EDEEEF] pl-8">
                          ...
                          <span className={`text-xs font-black uppercase tracking-tight ${status === 'Active' ? 'text-emerald-600' : 'text-amber-500'}`}>{status}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center mt-2">
                       <span className="text-[10px] font-bold text-[#767683] flex items-center gap-2 italic">
                        ...
                        <div className="flex gap-3">
                           <Link href={`/board/outlines/${item.id}/knowledge-tree`}>
                             <Button className="rounded-2xl bg-white text-[#1A237E] border border-[#E0E0FF] hover:bg-[#E0E0FF] h-11 px-8 font-black uppercase tracking-[0.2em] text-[9px] shadow-sm transition-all shadow-indigo-900/5">
                               {t('card.viewNodes')}
                             </Button>
                           </Link>
                        </div>
